...
/***/ (function(module, exports, __webpack_require__) {
	...
	var Bar = __webpack_require__(161);
	
	var TestBar = React.createClass({
	  displayName: 'TestBar',
	
	  render: function render() {
	    var data = [{ "value": '18' }, { "value": '8' }, { "value": '188' }, { "value": '86' }, { "value": '26' }, { "value": '35' }, { "value": '33' }, { "value": '19' }];
	    //定义的数据 调用组件传data:值，width:宽度，默认值50px color：柱子背景色，默认红色 title:柱状图标题 默认无
	    return React.createElement(Bar, { data: data, width: 40, color: '#ccc', title: '各组待处理工单', show: true });
	  }
	
	});
	
	ReactDOM.render(React.createElement(TestBar, null), document.getElementById('__react-content'));
	    ...
	    render: function render() {
	        ...
	        for (var j = 0; j < dataLen; j++) {
	            str.push(React.createElement(
	                ...
	                React.createElement(
	                    'span',
	                    { className: 'innerValue', style: { width: width + 'px', left: Number(20 + width) * j + Number(20) + 'px', height: Number(data[j].value) + Number(30) + 'px' } },
	                    data[j].value
	                ),
	                React.createElement('div', { className: 'inner', style: { width: width + 'px', backgroundColor: color, left: Number(20 + width) * j + Number(20) + 'px', height: data[j].value + 'px' } })
	            ));
	        }
	        //y轴刻度
	        for (var z = 0; z < Math.ceil(Number(maxValue) / 50) + 1; z++) {
	            {
	                this.props.show ? y.push(React.createElement(
	                    'div',
	                    { key: z },
	                    React.createElement(
	                        'span',
	                        { className: 'yname', style: { height: Number(50) * z + 'px' } },
	                        Number(50) * z
	                    ),
	                    React.createElement('label', { className: 'yLabel', style: { height: Number(50) * z + 'px' } })
	                )) : y.push(React.createElement(
	                    'div',
	                    { key: z },
	                    React.createElement(
	                        'span',
	        ...
	        return React.createElement(
	            ...
	            React.createElement(
	                'div',
	                { className: 'bar', style: { height: Number(maxValue + 20) + 'px' } },
	                y,
	                str
	            )
	        );
